...
<script lang="ts">
  ...
  const COLUMN_DEFS_UNTYPED = COLUMN_DEFS as ColumnDef[];
  ...
  let total = 2000;
  let rowData: Employee[] = generateData(2000);
  ...
  let statShowing    = 2000;
  let statTotal      = 2000;
  let statCols       = COLUMN_DEFS.length;
  let statSort       = 'none';
  let statHasSort    = false;
  let statSelected   = 0;
  let statFilterText = '';
  ...
  function updateStats(): void {
    ...
    statSort       = sort;
    statHasSort    = sortState.length > 0;
    statSelected   = selectedIds.length;
    statFilterText = filterText;
  }
  ...
  function onGridReady(api: GridApi): void {
    apiRef = api as GridApi<Employee>;
    updateStats();
  }
...
<div class="demo-bar">
  ...
  <div class="demo-controls">
    ...
    <button id="btn-help" on:click={() => helpOpen = true}>? Help</button>
  </div>
</div>
...
<div class={`help-backdrop${helpOpen ? ' open' : ''}`} on:click={() => helpOpen = false}>
  <!-- svelte-ignore a11y-click-events-have-key-events a11y-no-static-element-interactions -->
  <div class="help-modal" on:click|stopPropagation>
    <div class="help-modal-header">
      <h2>&#9889; Ultimate Data Grid — Quick Reference</h2>
      <button class="help-close" on:click={() => helpOpen = false}>&#10005;</button>
    </div>
    <div class="help-body">
